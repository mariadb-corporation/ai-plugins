...
OpenCode surfaces a relevant skill) and asserts the generated SQL takes the
MariaDB-preferred form the skill teaches. Assertions are regex/substring — stable
and cheap — rather than an LLM judge.

OpenCode is model-agnostic, but is most commonly run on Claude, so the eval
exercises a Claude model via the official `anthropic` SDK. Point EVAL_MODEL at
whatever model your key can reach to exercise a different one.
